...
public static unsafe class S
{
    ...
    public static nint LogBuffer = 0;
    public const int LogBufferLen = 256;
    public const int LogBufferSize = sizeof(char) * LogBufferLen;

    public static void InitLogBuffer() =>
        LogBuffer = C.malloc(LogBufferSize);

    public static void ClearLogBuffer() =>
        SimpleUtility.C.memset(LogBuffer, 0, LogBufferSize);

    public static void FreeLogBuffer() =>
        C.free(ref LogBuffer);
    ...
    public static void Log(string message)
    {
        ...
        span = new((void*)LogBuffer, LogBufferSize);

        len = Math.Min(LogBufferLen, message.Length);
        for (i = 0; i < len; i++)
            span[i] = message[i];
    }

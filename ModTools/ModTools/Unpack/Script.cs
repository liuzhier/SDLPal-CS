...
{
    static Dictionary<int, Address> _addressDict { get; set; } = [];
    ...
    record struct Address
    {
        public  string          Tag;            // 地址标签
        public  AddressType     Type;           // 地址类型
        public  int             ObjectId;       // 对象编号
    }
    ...
    public static string AddAddress(int address, string? addressName = null, AddressType type = AddressType.Public, int objectId = -1)
    {
        //
        // 若地址名称为空则自动生成
        //
        ...
        if (_addressDict.TryGetValue(address, out var addr))
        {
            //
            // 查找成功，返回字典里的地址标签
            //
    ...
    {
        void AddAddr(CScriptArgs arg) => AddAddress(arg.UShort);
        ...
        fileScenes = new StreamWriter[Record.Data.MaxScenes + 1];
        file = filePublic;
        ...
        for (i = 1; i <= end; i++)
        {
            ...
            if (i % progress == 0 || i == end)
                S.Log($"Unpack the game data. <Scirpt Addr: {((float)i / count * 100):f2}%>");
        ...
        for (i = 1; i <= end; i++)
        {
            ...
            // 检查脚本地址是否被注册
            //
            if (_addressDict.TryGetValue(i, out var address))
            {
                if (NeedSelectFile)
                {
                    ...
                    {
                        AddressType.Public => filePublic,
                        AddressType.Hero => fileHero,
                        AddressType.Item => fileItem,
                        AddressType.Magic => fileMagic,
                        AddressType.Enemy => fileEnemy,
                        AddressType.Poison => filePoison,
                        AddressType.Scene => fileScenes[address.ObjectId] ??= new StreamWriter($@"{pathOut}\src\Scene\{address.ObjectId:D5}.ts"),

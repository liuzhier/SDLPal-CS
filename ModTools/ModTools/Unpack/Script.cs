...
public static unsafe class Script
{
    ...
    public static void Init()
    {
        ...
        // 加载指令对应的函数名
        //
        S.JsonLoad(out Message.FunctionNames, $@"{pathDependency}\Func.json");
    ...
    public static void Process()
    {
        ...
        for (i = 1; i <= end; i++)
        {
            ...
            names = Message.FunctionNames.First(x => x.Contains($"0x{pThis->Command:X4}"));
            name = names[0];
        ...
        for (i = 1; i <= end; i++)
        {
            ...
            scriptText = null;
            _args = [];
            names = Message.GetFunctionName($"0x{pThis->Command:X4}");
            name = names[0];
            switch (name)
            {

...
public static unsafe class Hero
{
    ...
    public static void Process()
    {
        ...
        // 读取可习得的仙术, 并计算有多少组
        //
        (pNative, count) = PalUtil.ReadMkfChunk(Config.FileBase, 6);
        ...
        // 读取 Hero 数据块
        //
        (pNative2, _) = PalUtil.ReadMkfChunk(Config.FileBase, 3);
        ...
        for (i = (int)EntityBeginId.Hero, j = 0; i < end; i++, j++)
        {
            ...
            // 记录 Hero 名称
            //
            indexContent[j] = Message.GetEntityName(pBase->Name[j]);
            ...
            hero = new RGame.Hero(
                ...
                {
                    FriendDeathTag = Config.AddAddress(
                        Config.IsDosGame ?

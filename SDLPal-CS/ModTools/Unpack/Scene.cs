...
public static unsafe class Scene
{
    ...
    public static void Process()
    {
        ...
        ushort          status, mode, enumObstacle, enumTouchNear;
        bool            isObstacle, isAutoTrigger;
        ...
        // 处理 Scene 实体对象
        //
        enumObstacle = (ushort)EventState.Obstacle;
        ...
        for (i = 0; i < sceneCount; i++)
        {
            ...
            for (k = 0; j < eventEnd; j++, k++)
            {
                ...
                isObstacle = status >= enumObstacle;
                ...
                eventObject = new(
                    ...
                    Trigger: new(
                        StatusCode: (ushort)(isObstacle ? 0 : (status - enumObstacle)),
    ...
    public static (ushort, ushort) GetSoftSceneEventId(ushort originEventId)
    {
        ushort      sceneId, eventId;

        for (sceneId = 0; sceneId < SceneEventIndexs.Length; sceneId++)
            if (originEventId < SceneEventIndexs[sceneId])
                break;

        eventId = (ushort)(originEventId - SceneEventIndexs[--sceneId]);

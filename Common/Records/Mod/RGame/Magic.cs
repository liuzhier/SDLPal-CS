...
public class MagicBase(int Id)
{
    public int Id { get; set; } = Id;               // 实体编号
    [JsonIgnore]
    public Magic Magic => S.Entity.Magics[Id];      // 实际仙术对象
}
...
{
    public AddressBase Success { get; set; } = null!;     // 后序脚本（前序脚本成功才执行）
    public AddressBase Use { get; set; } = null!;         // 前序脚本（一般用于检查是否具备施法条件）

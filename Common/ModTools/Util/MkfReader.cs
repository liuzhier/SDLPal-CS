...
public unsafe class MkfReader(string path) : IDisposable
{
    ...
    FileReader Reader { get; init; } = new(path);
    ...
    public void Seek(long offset, SeekOrigin flag) => Reader.Seek(offset, flag);
    ...
    void SeekHeader(int chunkId) => Reader.Seek(chunkId * (int)UnitSize.MKF, SeekOrigin.Begin);

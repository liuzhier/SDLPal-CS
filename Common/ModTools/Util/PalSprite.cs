...
public unsafe class PalSprite((nint buffer, int size) data) : IDisposable
{
    ...
    byte* PtrInt8 => (byte*)Buffer;
    ushort* PtrInt16 => (ushort*)Buffer;
    public int PackCount => *PtrInt16;
    ...
    public int GetFrameSize(int packId)
    {
        ...
        // 获取指定包和下一个包的偏移量
        //
        iOffset = PtrInt16[packId] << 1;
        iNextOffset = PtrInt16[packId + 1] << 1;
    ...
    public nint GetFrame(int frameId)
    {
        ...
        // 获取帧偏移
        //
        if ((offset = (PtrInt16[frameId] << 1)) == 0x18444)

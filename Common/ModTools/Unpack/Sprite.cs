...
public static unsafe class Sprite
{
    ...
    public static void Init()
    {
        ...
        // 输出处理进度
        //
        UiUtil.Log("Unpack the game data. <Palette>");

        //
        // 创建输出目录 Palette
        //
        pathOut = $"{PalConfig.ModWorkPath.Assets.Palette}";
        COS.Dir(pathOut);
    ...
    public static void ProcessRng(string pathIn, string pathOut)
    {
        ...
        for (i = 0; i < count; i++)
        {
            ...
            for (j = 0; j < frameCount; j++)
            {
                ...
                (pPack, packSize) = PalUtil.Unpack(pRng, PalConfig.IsDosGame);
    ...
    public static void ProcessPack(string pathIn, string pathOut, int beginId = 0, int endId = -1, bool isCompressedPack = true, bool isFrameSequence = true, bool isRlePack = true, bool haveSubPath = true)
    {
        ...
        for (i = beginId; i <= endId; i++)
        {
            ...
            // 逐帧导出
            //
            sprite = (isEmptyImage) ? null! : new((isCompressedPack) ? PalUtil.Unpack(pChunk, PalConfig.IsDosGame) : (pChunk, packSize));

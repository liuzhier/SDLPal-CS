...
public static unsafe class Map
{
    ...
    public static void Init()
    {
        ...
        // 创建输出目录 MapData
        //
        pathOut = $"{PalConfig.ModWorkPath.Assets.MapData}";
        COS.Dir(pathOut);

        //
        // 将地图编辑器复制到工作目录
        //
        S.FileCopy("Dependency", PalConfig.ModWorkPath.Assets.MapData.MapEditorName, pathOut);
        ...
        //
        {
            ...
            // 读取 PAT.MKF
            //
            mkf = new(PalConfig.PalWorkPath.DataBase.Palette);
            ...
            // 创建阉割后的 PAT.MKF
            //
            fileOut = new(PalConfig.ModWorkPath.Assets.MapData.Palette);
    ...
    public static void Process()
    {
        ...
        // 输出处理进度
        //
        UiUtil.Log("Unpack the game data. <MapData>");

        //
        // 创建输出目录 MapData
        //
        pathMap = PalConfig.ModWorkPath.Assets.MapData.Map;
        COS.Dir(pathMap);
        pathTile = PalConfig.ModWorkPath.Assets.MapData.Tile;
        COS.Dir(pathTile);

        //
        // 打开地图数据和地图瓦片文件
        //
        mkfMap = new(PalConfig.PalWorkPath.DataBase.Map);
        mkfTile = new(PalConfig.PalWorkPath.Sprite.Tile);
        ...
        for (i = 1; i < size; i++)
        {
            ...
            {
                pFree = pBinary;
                (pBinary, len) = PalUtil.Unpack(pBinary, PalConfig.IsDosGame);

...
public static unsafe class PalUiGame
{
    ...
    public static float StretchFactor => (PalVideo.Height / 480f) > 1 ? PalVideo.Height / 480f : 1;
    ...
    /// <param name="uiName">UI 名称</param>
    /// <param name="surfaceId">贴图编号</param>
    public static (int W, int H) GetUiSize(nint[] ui, int surfaceId = 0, float stretchFactor = -1)
    {
        var (w, h) = GetUiOriginSize(ui, surfaceId);

        if (stretchFactor < 0) stretchFactor = StretchFactor;
    ...
    /// <param name="uiName">UI 名称</param>
    /// <param name="surfaceId">贴图编号</param>
    public static (int W, int H) GetUiSize(string uiName, int surfaceId = 0, float stretchFactor = -1) =>
    ...
    public static int TitleMenu()
    {
        ...
        while (true)
        {
            ...
            //});
            var movie = COS.Texture(PalGlobal.MoviePlayer.MovieSurface, needFree: false);
            PalScreen.CopyMain(movie);
            FOS.Texture(movie);
    ...
    public static bool CommandMenu(Atlas atlas, CommandAnimationType animationType = CommandAnimationType.CenterButton, int frameId = 0, bool isReverse = false)
    {
        ...
        // 加载图像
        //
        var box = COS.Texture(ui[0], lockedMod: 2, needFree: false);
        var nextGroupButton = COS.Texture(ui[1], needFree: false);
        var border = COS.Texture(ui[2], needFree: false);
        var center = COS.Texture(ui[3], needFree: false);
        var bottom = COS.Texture(ui[6], needFree: false);
        var left = COS.Texture(ui[9], needFree: false);
        var top = COS.Texture(ui[12], needFree: false);
        var right = COS.Texture(ui[15], needFree: false);
        ...
        if (isReverse)
        {
            ...
            switch (animationType)
            {
                ...
                case CommandAnimationType.OtherButton:
                    ...
                    FOS.Texture(center);
                    center = COS.Texture(ui[4], needFree: false);
                    break;
    ...
    {
        var magics = S.Save.Entity.Heroes[heroId].Magic.Learned;

        //
        // 统计有效仙术数量
        //
        var actualMagicsCount = 0;
        for (var i = 0; i < Base.MaxHeroMagic; i++) if (magics[i].Id > 0) actualMagicsCount++;
    ...
    public static void UseItemBackground(Atlas atlas, int heroId = 0)
    {
        ...
        else
        {
            ...
            var magic = S.Entity.Heroes[heroId].Magic;
            cursorId = magic.MagicCursorId;

            //
            // 获取仙术描述
            //
            desc = S.Entity.Magics[magic.Learned[cursorId].Id].Description;
        ...
        while (!needExit)
        {
            ...
            else if (PalInput.Pressed(PalKey.Search))
            {
                ...
                else
                {
                    ...
                    item.Script.Use.Value = PalScript.RunTrigger(item.Script.Use.Value, -1, -1, $"UseItem<{item.Name}>");
    ...
    public static bool ConfirmMenu(ref Atlas atlas)
    {
        ...
        PalText.DrawTexts(atlas, [
            ...
            new()
            {
                ...
                PosOffset = new()
                {
                    Y = (int)(17 * backgroundPack.StretchFactor),
                },
                Foreground = ColorGold,
                HorizontalAlign = PalHorizontalAlign.Middle,
                ParentPack = backgroundPack,

...
public static unsafe class PalDialog
{
    public const int
    ...
    static DialogPosition Position { get; set; }
    ...
    static int MaxRowId => 3 + ((HaveName && (Position == DialogPosition.Top)) ? 1 : 0);
    ...
    /// <param name="color">RGB 颜色值，缺省则为白色</param>
    /// <param name="isPlayingRng">是否正在播放 Rng 动画，缺省则为否</param>
    public static void InitMesssge(DialogPosition position, uint color = DefaultColor, int faceId = 0, bool isPlayingRng = false) =>
        InitMesssge((Position = position) switch
        {
            DialogPosition.Top => TopPos,
            DialogPosition.Middle => MiddlePos,
            DialogPosition.Bottom => BottomPos,
    ...
    /// 初始化对话框默认信息
    /// </summary>
    public static void InitDefaultMesssge() => InitMesssge(DialogPosition.Top);
    ...
    public static void ClearMesssge(bool waitForKey, bool needDeleteName = true)
    {
        ...
        if ((Position != DialogPosition.Top) && (Position != DialogPosition.Bottom))
            //
            // 初始化对话框默认信息
            //
            InitDefaultMesssge();
    ...
    static AtlasPack DrawDialogAnimation(Atlas atlas)
    {
        var pack = PalUiGame.DrawBox(atlas, new()
        {
            ...
            {
                DialogPosition.Top => PalVerticalAlign.Top,
                DialogPosition.Bottom => PalVerticalAlign.Bottom,
        ...
        if (FaceId > 0)
        {
            ...
            // 绘制肖像
            //
            var surface = COS.Surface($@"{S.ModSpritePath.Avatar}\{FaceId:D5}.png");
            var stretchFactor = 4;
            ...
            atlas.Add(new(surface, needFree: true), new()
            {
                Rect = new()
                {
                    X = Position switch
                    {
                        DialogPosition.Bottom => w,
                        DialogPosition.Top or _ => -w,
                    },
                    Y = Position switch
                    {
                        DialogPosition.Top => -pack.Rect.Y,
                        DialogPosition.Bottom => -pack.Rect.Y,
                        _ => 0,
                    },
                },
                ...
                {
                    DialogPosition.Bottom => PalHorizontalAlign.Right,
                    DialogPosition.Top or _ => PalHorizontalAlign.Left,
                },
                VerticalAlign = Position switch
                {
                    DialogPosition.Bottom => PalVerticalAlign.Bottom,
                    DialogPosition.Top or _ => PalVerticalAlign.Top,
                },
                ParentPack = pack,
            });
        }
    ...
    public static void DrawTalkText(string text)
    {
        ...
        if (CurrentRowId == 0)
            ...
            {
                DialogPosition.Top => h,
                DialogPosition.Bottom => -h,
                _ => 0,
            };
        ...
        if ((CurrentRowId == 0) && !HaveName && (Position == DialogPosition.Bottom))
            //
            // 防止没有名称时行数过多
            //
            CurrentRowId++;
        ...
        // 有名称的话，正文需要全部后移两个字
        //
        currentRowInfo.PosOffset.X = (!HaveName || (HaveName && IsName) || (Position == DialogPosition.Middle)) ? 0 : offsetX;
        ...
        BeginDrawChar:
            ...
            if (((Position == DialogPosition.Top) || (Position == DialogPosition.Bottom)))
            {
                ...
                {
                    DialogPosition.Bottom => MaxRowId,
                    DialogPosition.Top or _ => MaxRowId,
                };

...
public static unsafe class PalScreen
{
    ...
    public static void Init()
    {
        ...
        // 制作淡入淡出遮罩，并先把它隐藏起来
        //
        SDL.SetRenderDrawColor(PalVideo.Renderer, 0x00, 0x00, 0x01, 0xFF);
    ...
    public static void FadeAndUpdate(int delay, bool isFadeOut = true, byte stepCount = 2)
    {
        ...
        if (isFadeOut)
        {
            for (var alpha = 0f; ; alpha += addNum)
            {
                var time = SDL.GetTicks() + (ulong)delay;
    ...
    public static void FadeToScreen(int delay, byte stepCount = 5)
    {
        var addNum = stepCount / 255.0f;
    ...
    /// <param name="stepCount">总步长</param>
    public static void FadeToColor(int delay, uint color = 0x000000, bool isFadeOut = true, bool updateScene = false, byte stepCount = 5)
    {
        var alpha = 0.0f;
        var addNum = stepCount / 255.0f;

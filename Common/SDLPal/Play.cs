...
public static class PalPlay
{
    ...
    public static void GameUpdate(bool allowTrigger)
    {
        ...
        if (allowTrigger)
        {
            ...
            if (PalGlobal.EnterScene)
            {
                ...
                if (address.Value != 0)
                {
                    ...
                    address.Value = PalScript.RunTrigger(address.Value, -1, -1, $"SceneEnter<{PalGlobal.Save.SceneId}>");
            ...
            for (var i = 0; i < scene.Events.Count; i++)
            {
                ...
                if (@event.Trigger.VanishTime != 0)
                {
                    //
                    // 更新所有事件对象的消失时间/僵直（逃跑后罚站）时间
                    //
                    @event.Trigger.VanishTime += (@event.Trigger.VanishTime < 0 ) ? 1 : -1;
                    continue;
                }
                ...
                else if (@event.IsDisplay && @event.Trigger.IsAutoTrigger)
                {
                    ...
                    if (Math.Abs(xOffset) + Math.Abs(yOffset) * 2 < (@event.Trigger.Range - 1) * 32 + 16)
                    {
                        ...
                        @event.Script.Trigger.Value = PalScript.RunTrigger(address.Value, -1, i);
        ...
        for (var i = 1; i < scene.Events.Count; i++)
        {
            ...
            if (@event.IsDisplay && (@event.Trigger.VanishTime == 0))
            {
                var address = @event.Script.Auto;

                if (address.Value != 0)
                {
                    address.Value = PalScript.RunAuto(address.Value, i);
    ...
    public static void Search()
    {
        ...
        for (var i = 0; i < 13; i++)
        {
            ...
            for (var k = 1; k < events.Count; k++)
            {
                ...
                {
                    trail.FrameId = 0;                                          // 当前站立姿势
                    trail.Direction = (PalDirection)((int)(dir + 2) % 4);       // 面朝领队
                ...
                // 执行事件触发脚本
                //
                @event.Script.Trigger.Value = PalScript.RunTrigger(@event.Script.Trigger.Value, -1, k);
    ...
    public static void StartFrame()
    {
        ...
        else if (PalInput.Pressed(PalKey.ThrowItem))
        {
            PalScene.CanFly = !PalScene.CanFly;
        }

...
public static unsafe class PalScene
{
    public const int
        Fps            = 15,
        FrameTime      = (1000 / Fps);
    ...
    {
        foreach (var heroTeam in S.Save.HeroTeams)
            UpdateCharacterGestures(isWalking, heroTeam.Trail, S.Entity.Heroes[heroTeam.HeroId].Raw->FramesPerDirection);
    ...
    public static void UpdateHeroTeamPos(Pos posTarget)
    {
        var heroTeams = S.Save.HeroTeams;
        var followers = S.Save.Followers;
        ...
        //
        for (var i = 1; i < heroTeams.Length; i++)
        {
        ...
        // 最后更新领队的位置
        //
        S.SetMemberPos(0, posTarget);
        S.SetMemberDirection(0, S.GetHeroTeamDirection());
    }
    ...
    public static void HeroTeamWalkTo(BlockPos bpos, int speed)
    {
        ...
        while (true)
        {
            ...
            if (Math.Abs(offsetX) <= speed * 2)
            {
                pos.X += offsetX;
            }
            else
            {
                pos.X += speed * (offsetX < 0 ? -2 : 2);
            }

            if (Math.Abs(offsetY) <= speed)
            {
                pos.Y += offsetY;
            }
            else
            {
                pos.Y += speed * (offsetY < 0 ? -1 : 1);
            }
    ...
    public static void PartyRideEventObject(int sceneId, int eventId, BlockPos bpos, int speed)
    {
        ...
        while (xOffset != 0 || yOffset != 0)
        {
            ...
            t = (uint)(SDL.GetTicks() + PalScene.FrameTime);

            if (yOffset < 0) S.SetHeroTeamDirection((xOffset < 0) ? PalDirection.West : PalDirection.North);
            else S.SetHeroTeamDirection((xOffset < 0) ? PalDirection.South : PalDirection.East);
    ...
    static void DrawSprite(Atlas atlas)
    {
        ...
        for (var i = 0; i < len; i++)
        {
            ...
            var hero = S.Entity.Heroes[heroTeams[i].HeroId];
            var walkFrames = hero.Raw->FramesPerDirection;
            ...
            var needStretch = PalResource.CheckStretch(hero.Raw->SpriteId);
            if (!needStretch)
            {
                //
                // 不需要缩放的贴图，应使用其逻辑高度

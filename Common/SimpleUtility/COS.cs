...
public unsafe static class COS
{
    ...
    public static nint Texture(nint surface, int lockedMod = 0, bool needFree = true)
    {
        nint                texture;

        texture = TextureL(surface, needFree);
        ...
        if (lockedMod != 0)
        {
            ...
            if (needFree) FOS.Surface(ref surface);
            if (needFree) FOS.Texture(ref texture);

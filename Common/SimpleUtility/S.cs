...
public static unsafe class S
{
    ...
    private static ReadOnlySpan<char> CheckHex(string text, out NumberStyles numberStyles)
    {
    ...
    public static int StrToInt(string strVal)
    {
        int                     val;
        ReadOnlySpan<char>      span;
        NumberStyles            numberStyles;

        span = CheckHex(strVal, out numberStyles);

        Failed(
           "S.StrToUshort",
            $"The string '{strVal}' is not a number",
            int.TryParse(span, numberStyles, CultureInfo.InvariantCulture, out val)
        );

        return val;
    }

    public static int StrToInt(char strVal) =>
        StrToInt(strVal.ToString());

    public static ushort StrToUShort(string text)
    {
        ushort              val;
        NumberStyles        numberStyles;
        ...
        Failed(
           "S.StrToUshort",
           $@"The string '{text}' is not a number",
           ushort.TryParse(CheckHex(text, out numberStyles), CultureInfo.InvariantCulture, out val)
        );

        return val;
    }

    public static ushort StrToUShort(char character) =>
        StrToUShort(character.ToString());
}

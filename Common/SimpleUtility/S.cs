...
public static unsafe class S
{
    ...
    public static void DirCopy(string sourceDirectory, string fileFullName, string destDirectory, bool checkPath = true)
    {
        ...
        foreach (string file in Directory.EnumerateFiles(sourceDirectory, fileFullName))
            FileCopy(sourceDirectory, Path.GetFileName(file), destDirectory);

        foreach (string directory in Directory.EnumerateDirectories(sourceDirectory, fileFullName))
        {
            var subPathName = Path.GetFileName(directory)!;
            DirCopy(Paths(sourceDirectory, subPathName), "*", Paths(destDirectory, subPathName), checkPath);
        }
    }
    ...
    /// 弹出警告消息框。
    /// </summary>
    /// <param name="funcName">触发警告的函数名</param>
    /// <param name="error">警告信息内容</param>
    /// <param name="isCorrect">断言触发器</param>
    public static Exception Warning(string funcName, string warning, bool isCorrect = false)
    {
        //
        // 播放警告音效
        //
        PalAudio.PlayVoice(1002);
        ...
        var logFatal = $"{funcName} failed: {warning}";

        if (warning.Last() != '.') logFatal += '.';
        ...
        if (SDLWindow == 0) MessageBox(ExeHwnd, logFatal, title);
        else
        {
    ...
    /// <param name="isCorrect">断言触发器</param>
    public static Exception Failed(string funcName, string error, bool isCorrect = false)
    {
        //
        // 如果没有触发错误，直接退出
        //
    ...
    public static Member GetMember(Index memberId) => Save.Members[memberId];
    public static Hero GetMemberHero(Index memberId) => Save.Members[memberId].Hero;
    ...
    public static Trail GetEventTrail(int sceneId, int eventId) => GetEvent(sceneId, eventId).Trail;
    ...
    public static void AutoUseItem()
    {
        if (CurrentItem.Scope.Consuming && PalScript.ScriptSuccess)
        {
            ...
            // 使用成功了，消耗一个道具
            //
            Save.AddInventory(CurrentInventory.ItemId, -1);
        }
    }
}
